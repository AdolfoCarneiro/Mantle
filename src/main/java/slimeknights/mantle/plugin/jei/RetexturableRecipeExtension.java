 ...
 * Stateless singleton: JEI now passes the recipe into each method instead of wrapping one instance per recipe.
 */
...
//    guiItemStacks.addTooltipCallback(this);
    ...
    List<IRecipeSlotBuilder> inputSlots = craftingGridHelper.createAndSetInputs(builder, VanillaTypes.ITEM_STACK, inputStacks, width, height);
    ...
    if (inputSlots.size() != 9) {
      ...
      builder.createFocusLink(Streams.concat(Stream.of(output), Arrays.stream(textureSlots).mapToObj(i -> inputSlots.get(MantleJEIConstants.getCraftingIndex(i, width, height)))).toArray(IRecipeSlotBuilder[]::new));
    }
  }
