...
public class BookLoader implements ResourceManagerReloadListener {
  ...
  public static void updateSavedPage(@Nullable Player player, InteractionHand hand, String page) {
    if (player != null) {
      ...
      if (!item.isEmpty()) {
        ...
        MantleNetwork.sendToServer(new UpdateHeldPagePacket(hand, page));
      }
    }
  }
  ...
  public static void updateSavedPage(@Nullable Player player, int slot, String page) {
    if (player != null && slot >= 0) {
      ...
      if (!item.isEmpty()) {
        BookHelper.writeSavedPageToBook(item, page);
        MantleNetwork.sendToServer(new UpdateInventoryPagePacket(slot, page));
      }
    }
  }
  ...
  public static void updateSavedPage(BlockPos pos, String page) {
    MantleNetwork.sendToServer(new UpdateLecternPagePacket(pos, page));
  }
