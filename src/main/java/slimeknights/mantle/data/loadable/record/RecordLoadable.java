...
public interface RecordLoadable<T> extends Loadable<T> {
  ...
  /** Creates a MapCodec backed by this RecordLoadable */
  default MapCodec<T> mapCodec(TypedMap context) {
    return new MapCodec<T>() {
      @Override
      public <O> com.mojang.serialization.DataResult<T> decode(com.mojang.serialization.DynamicOps<O> ops, com.mojang.serialization.MapLike<O> input) {
        try {
      ...
      @Override
      public <O> java.util.stream.Stream<O> keys(com.mojang.serialization.DynamicOps<O> ops) { return java.util.stream.Stream.empty(); }
    };
  }
