...
public record ConditionalLoadable<T extends IHaveLoader>(GenericLoaderRegistry<T> registry, @Nullable T defaultIfFalse) implements RecordLoadable<T> {
  ...
  public T deserialize(JsonObject json, TypedMap context) {
    ...
    if (conditionsElement != null) {
      conditionsMatch = ICondition.LIST_CODEC.parse(JsonOps.INSTANCE, conditionsElement).getOrThrow(JsonParseException::new)
  ...
  public void serialize(T object, JsonObject json) {
    ...
    json.add("conditions", ICondition.LIST_CODEC.encodeStart(JsonOps.INSTANCE, List.of(conditional.conditions())).getOrThrow());
